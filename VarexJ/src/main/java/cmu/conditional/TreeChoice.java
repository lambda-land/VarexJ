...
/**
 * Choice implementation as tree. Each node represents a decision as
 * {@link FeatureExpr} between two conditionals.
 * 
    ...
    if (tb instanceof One) {
        if (eb instanceof TreeChoice) {
            if (((TreeChoice<T>) eb).thenBranch.equals(tb)) {
                return createInstance(featureExpr.or(featureExpr.not().and(((TreeChoice<T>) eb).featureExpr)), tb,
                        ((TreeChoice<T>) eb).elseBranch);
            }
            if (((TreeChoice<T>) eb).elseBranch.equals(tb)) {
                return createInstance(featureExpr.or(featureExpr.not().and(((TreeChoice<T>) eb).featureExpr.not())),
                        tb, ((TreeChoice<T>) eb).thenBranch);
            }
        }
    }
    if (eb instanceof One) {
        if (tb instanceof TreeChoice) {
            if (((TreeChoice<T>) tb).thenBranch.equals(eb)) {
                return createInstance(featureExpr.not().or(featureExpr.and(((TreeChoice<T>) tb).featureExpr)), eb,
                        ((TreeChoice<T>) tb).elseBranch);
            }
            if (((TreeChoice<T>) tb).elseBranch.equals(eb)) {
                return createInstance(featureExpr.not().or(featureExpr.and(((TreeChoice<T>) tb).featureExpr.not())),
                        eb, ((TreeChoice<T>) tb).thenBranch);
            }
        }
    }
...
public static <T> Conditional<T> createInstance(FeatureExpr featureExpr, Conditional<T> thenBranch,
    ...
    if (thenBranch instanceof TreeChoice) {
        ...
        } else if (not.equivalentTo(((TreeChoice) thenBranch).featureExpr)) {
            thenBranch = ((TreeChoice) thenBranch).elseBranch;
        }
    }
    ...
    if (elseBranch instanceof TreeChoice) {
        ...
        } else if (not.equivalentTo(((TreeChoice) elseBranch).featureExpr)) {
            elseBranch = ((TreeChoice) elseBranch).thenBranch;
        }
    }

    if (thenBranch instanceof TreeChoice) {
        if (((TreeChoice) thenBranch).elseBranch.equals(elseBranch)) {
            return new TreeChoice(((TreeChoice) thenBranch).featureExpr.and(featureExpr),
                    ((TreeChoice) thenBranch).thenBranch,  ((TreeChoice) thenBranch).elseBranch);
        }

        if (((TreeChoice) thenBranch).thenBranch.equals(elseBranch)) {
            return new TreeChoice(((TreeChoice) thenBranch).featureExpr.orNot(featureExpr),
                    ((TreeChoice) thenBranch).thenBranch,  ((TreeChoice) thenBranch).elseBranch);
        }
    }
    ...
    if (elseBranch instanceof TreeChoice) {
        if (((TreeChoice) elseBranch).elseBranch.equals(thenBranch)) {
            return new TreeChoice(((TreeChoice) elseBranch).featureExpr.andNot(featureExpr),
                    ((TreeChoice) elseBranch).thenBranch,  ((TreeChoice) elseBranch).elseBranch);
        }

        if (((TreeChoice) elseBranch).thenBranch.equals(thenBranch)) {
            return new TreeChoice(((TreeChoice) elseBranch).featureExpr.or(featureExpr),
                    ((TreeChoice) elseBranch).thenBranch,  ((TreeChoice) elseBranch).elseBranch);
        }
    }
