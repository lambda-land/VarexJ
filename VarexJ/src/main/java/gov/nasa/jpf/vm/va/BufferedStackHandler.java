...
public class BufferedStackHandler extends StackHandler implements Cloneable, IStackHandler {
    ...
    public void debufferAll() {
        ...
        if(!buffer.isEmpty()) {
            ...
            if(!flag) {
                flag = true;
                //System.out.println("flagNum " + flagNum + " " + ctx);
                flagNum++;
            }
            debufferNum++;
        }
    ...
    public void storeOperand(FeatureExpr ctx, int index) {
        if (!buffer.isEmpty()) {
            ...
            //System.out.println("storeOperand " + ctx + " " + bufferCTX);
            debufferAll();
        }
    ...
    public void push(FeatureExpr ctx, Object value, boolean isRef) {
        ...
        if (buffer.isEmpty()) {
            ...
            //System.out.println("push " + ctx + " " + bufferCTX);
    ...
    public <T> Conditional<T> pop(FeatureExpr ctx, Type t) {
        if (!buffer.isEmpty()) {
            if (bufferCTX.equivalentTo(ctx)) {
                ...
                //System.out.println("pop " + ctx + " " + bufferCTX);
    ...
    public Conditional<Integer> peek(FeatureExpr ctx) {
        if (!buffer.isEmpty()) {
            ...
            //System.out.println("peek " + ctx + " " + bufferCTX);
            debufferAll();
        }
                ...
                while (n <= pointer) {
                    ...
                    if (type instanceof Double || type instanceof Long) {
                        ...
                        if (pointer - n > 1) {
                            n++;
                            pointer--;
                            //System.out.println("peekDL " + n + " " + offset + " " + ctx + " " + bufferCTX);
                            //debufferAll();
                            ...
                            return super.peek(ctx, offset);
//                            if(n == pointer) {
//                                return value.map(new Function<Number, Integer>() {
//                                 
//                                    @Override
//                                    public Integer apply(Number x) {
//                                        long t;
//                                        if(x instanceof Double) {
//                                            t = Double.doubleToLongBits((Double)x);
//                                        } else {
//                                            t = ((Long)x).longValue();
//                                        }
//                                        return (int)t;
...
//                            return super.peek(ctx, offset);
    ...
    public void storeLongOperand(FeatureExpr ctx, int index) {
        //System.out.println("storeLongOperand " + ctx + " " + bufferCTX);
    ...
    public void clear(FeatureExpr ctx) {
        if (!buffer.isEmpty()) {
            if (bufferCTX.equivalentTo(ctx)) {
                ...
                //System.out.println("clear " + ctx + " " + bufferCTX);
                debufferAll();
            }
        }
        super.clear(ctx);
    ...
    public boolean hasAnyRef(FeatureExpr ctx) {
        if (!buffer.isEmpty()) {
            ...
            //System.out.println("hasAnyRef " + ctx + " " + bufferCTX);
            debufferAll();
        }
        return super.hasAnyRef(ctx);
    }
    ...
    public void set(FeatureExpr ctx, int offset, int value, boolean isRef) {
        //System.out.println("set " + ctx + " " + bufferCTX);
        debufferAll();
    ...
    public void pushLocal(FeatureExpr ctx, int index) {
        ...
        for (Entry v : value.toList()) {
            if (isRef == null) {
                ...
                //System.out.println("pushLocal " + ctx + " " + bufferCTX);
                debufferAll();
        ...
        if (!buffer.isEmpty()) {
            if (bufferCTX.equivalentTo(ctx)) {
                if (getBufferSize() > offset) {
                    ...
                    return buffer.get(n).isRef;
                } else {
                    return super.isRef(ctx, offset - buffer.size());
                }
            }
            //System.out.println("isRef " + ctx + " " + bufferCTX);
    ...
    public boolean isRefLocal(FeatureExpr ctx, int index) {
        if (index >= locals.length) {
            //System.out.println("isRefLocal " + ctx + " " + bufferCTX);
    ...
    public int valueSize(Object o) {
        if(o instanceof Conditional) return valueSize(((Conditional)o).getValue(true));
        if(o instanceof Double) return 2;
    ...
    public void dup(FeatureExpr ctx) {
        if (!buffer.isEmpty()) {
            if (bufferCTX.equivalentTo(ctx)) {
                ...
                if (valueSize(peek.value) == 1) {
                //if(buffer.peek().value.getValue(true) instanceof Integer) {
                    buffer.push(peek);
                    return;
                }
                //System.out.println("wow dup " + ctx + " " + bufferCTX + " " + peek.value.getValue().getClass());
                debufferAll();
            } else {
                //System.out.println("dup " + ctx + " " + bufferCTX);
                debufferAll();
            }
        }
    ...
    public void dup2(FeatureExpr ctx) {
        if (!buffer.isEmpty()) {
            ...
            //System.out.println("dup2 " + ctx + " " + bufferCTX);
            debufferAll();
        }
        super.dup2(ctx);
    }
    ...
    public void swap(FeatureExpr ctx) {
        if (!buffer.isEmpty()) {
            ...
            //System.out.println("swap " + ctx + " " + bufferCTX);
            debufferAll();
        }
        super.swap(ctx);
    }
    ...
    public void dup2_x1(FeatureExpr ctx) {
        if (!buffer.isEmpty()) {
            ...
            //System.out.println("dup2_x1 " + ctx + " " + bufferCTX);
            debufferAll();
        }
        super.dup2_x1(ctx);
    }
    ...
    public void dup2_x2(FeatureExpr ctx) {
        if (!buffer.isEmpty()) {
            ...
            //System.out.println("dup2_x2 " + ctx + " " + bufferCTX);
            debufferAll();
        }
        super.dup2_x2(ctx);
    }
        ...
        if (!buffer.isEmpty()) {
            if (buffer.size() >= 2) {
                if (bufferCTX.equivalentTo(ctx)) {
                    ...
                    if(valueSize(buffer.peek().value) == 1 && valueSize(buffer.get(1).value) == 1) {
                        buffer.add(2, buffer.peek());
                        return;
                    }
                }
            }
            //System.out.println("dup_x1 " + ctx + " " + bufferCTX + " " + buffer.peek().value.getValue().getClass());
            debufferAll();
        }
        super.dup_x1(ctx);
    }
    ...
    public void dup_x2(FeatureExpr ctx) {
        if (!buffer.isEmpty()) {
            if (buffer.size() >= 3 && bufferCTX.equivalentTo(ctx)) {
                ...
                //System.out.println("dup_x2 " + ctx + " " + bufferCTX);
                debufferAll();
            }
        }
        super.dup_x2(ctx);
    ...
    public Conditional<Integer> getLocal(FeatureExpr ctx, int index) {
        if (index >= locals.length) {
            //System.out.println("getLocal " + ctx + " " + bufferCTX);
            debufferAll();
        }
    ...
    @Override
    public int[] getSlots() {
        //System.out.println("getSlots " + " " + bufferCTX);
        debufferAll();
    ...
    public int[] getSlots(FeatureExpr ctx) {
        //System.out.println("getSlots " + ctx + " " + bufferCTX);
        debufferAll();
    ...
    public Conditional<Stack> getStack() {
        //System.out.println("getStack " + " " + bufferCTX);
        debufferAll();
