...
class VStack implements IVStack {
  ...
  /*
   * public void getStackWidth(final int count){ Conditional<Entry> slot2; slot2
   * = slots[0]; for(int i = 1; i <= count; i++){ slot2 =
   * slot2.mapfr(FeatureExprFactory.True(), new BiFunction<FeatureExpr, Entry,
   * Conditional<Entry>>() { public Conditional<Entry> apply(FeatureExpr c,
   * final Entry x) { if(x ) { return new One<>(true); } } } } }
   */
  public VStack() {
    size = -1;
   ...
   * Conditional<Integer> [] clone = (Conditional<Integer>[]) new
   * Conditional[this.size+1]; for(int i = 0; i <= this.size; i++){ [i] =
   * slots[i].map(new Function< Entry, Integer>(){ public Integer apply(Entry e)
   * { if(e == null) { return (Integer)null; } else { return e.value; } }
   * }).simplify(); } return clone; }
   */
  /**
  ...
  public void pushEntry(final FeatureExpr ctx, final Conditional<Entry> value) {
    ...
    // slots[++size] = ChoiceFactory.create(ctx, value,
    // (Conditional<Entry>)One.NULL);
  }
  ...
  public void push(final FeatureExpr ctx, final Object value, final boolean isRef) {
    ...
    if (value instanceof Conditional) {
      Conditional<Entry> tmp = ((Conditional<Object>) value)
          .mapfr(ctx, new BiFunction<FeatureExpr, Object, Conditional<Entry>>() {
            ...
            public Conditional<Entry> apply(final FeatureExpr f, final Object value) {
              ...
              if (value instanceof Integer) {
                ...
                /*
                 * long v = ((Long) value).longValue(); e = Entry.create((int)
                 * (v & 0xFFFFFFFFL), isRef); add(f, (int) (v >> 32), isRef);
                 */
                ...
                /*
                 * long v = Double.doubleToLongBits((Double) value); e =
                 * Entry.create((int) (v & 0xFFFFFFFFL), isRef); add(f, (int) (v
                 * >> 32), isRef);
                 */
  ...
  /**
   * stack size related operations: topSizeHelper, topSize, topSet
   */
  ...
  /**
   * get top elements of stacks related operations: gettopHelper, gettop
   */
  ...
  /*
   * public void seRef(final FeatureExpr ctx, final int index, final boolean
   * ref) { Conditional<Integer> offset = index2offset(ctx, index);
   * //System.out.println("offset " + offset); fillholes(); offset.mapfr(ctx,
   * new VoidBiFunction<FeatureExpr, Integer>() { public void apply(FeatureExpr
   * c, Integer offset) { //System.out.println("offset " + c + " " + offset);
   * if(Conditional.isContradiction(c) || offset == null) { return; } else {
   * if(offset > size) return ; slots[size - offset] = slots[size -
   * offset].mapfr(c, new BiFunction<FeatureExpr, Entry, Conditional<Entry>>() {
   * public Conditional<Entry> apply(FeatureExpr c, Entry e) {
   * //System.out.println(size + " " + c + " " + e);
   * if(Conditional.isContradiction(c) || e == null) { return new One<>(e); }
   * else { Entry ne = e.copy(); ne.isRef = ref; return ChoiceFactory.create(c,
   * new One<>(ne), new One<>(e)); } } }).simplify(); } } }); }
   */
...
/*
 * class Entry { boolean isRef = false; final int value;
 * 
 * // final Integer attr;
 * 
 * Entry copy() { return new Entry(value, isRef); }
 * 
 * public Entry(int value, boolean isRef) { this.value = value; this.isRef =
 * isRef; }
 * 
 * @Override public String toString() { if (isRef) { return " <" + value + "> ";
 * } return " [" + value + "] "; }
 * 
 * @Override public boolean equals(Object o) { if(o != null){ return ((Entry)
 * o).value == value && ((Entry) o).isRef == isRef; } else return false; }
 * 
 * @Override public int hashCode() { return value; }
 * 
 * static Entry[] references = new Entry[128]; static Entry[] values = new
 * Entry[128];
 * 
 * static { for (int i = 0; i < 128;i++) { references[i] = new Entry(i - 1,
 * true); values[i] = new Entry(i - 1, false); } }
 * 
 * static Entry create(int value, boolean isRef) { if (value >= -1 && value <
 * 127) { if (isRef) { return references[value + 1]; } else { return
 * values[value + 1]; } } return new Entry(value, isRef); } }
 */
