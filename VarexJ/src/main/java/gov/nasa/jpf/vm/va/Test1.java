...
public class Test1 {
    ...
    public static long[] testBufferedStack(int stackSize, NonStaticFeature[] options, int feComplexity, double ratio,
        ...
        */
            ...
            LinkedList<Integer> popCtx = new LinkedList<>();
            for(int i = 0; i < (int)(popNum * possibility); i++) {
               popCtx.add(1);
            }
            for(int i = 0; i < popNum - popNum * possibility; i++) popCtx.add(0);
            Collections.shuffle(popCtx);
            
            if(flag) {
                for(int i = 0; i < popCtx.size(); i++) {
                    System.out.print(popCtx.get(i) + " ");
                }
            }
            System.out.println("");
    ...
    public static void main(String[] args) {
        ...
        int n = 20, m = 2, nums = 1;
        NonStaticFeature[] options = Main.getOptions(10);
        ...
        for (int i = 0; i <= 10; i++) {
            ...
            for (int j = 0; j < nums; j++) {
                ...
                ans = testBufferedStack(200, options, randomFEComlexity, i*0.5, conditionalSize, operationsNum, 0.9);
